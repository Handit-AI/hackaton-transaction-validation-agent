...
This folder contains the implementation for the **velocity_checker** LLM node of the risk_manager agent.
...
The velocity_checker LLM node is responsible for:
...
```python
from src.nodes.llm.velocity_checker.processor import VelocityCheckerLLMNode

# Initialize the LLM node
node = VelocityCheckerLLMNode(config)
